...
## 6.7 MODE: SIDE QUESTS — Génération structurée de quêtes secondaires (S×L)

### Objectif

Ce mode génère des quêtes secondaires cohérentes, jouables et alignées avec la campagne,

sans inventer d’univers. Le moteur utilise uniquement :

- le **SPINE** (sources existantes : scènes, lieux, PNJ, interludes),
- le **SideQuest Ledger** (historique),
- et, si présentes, les **références** (qui modulent le ton, le rythme ou certains motifs).
---

### 6.7.1 Préconditions

Pour activer ce mode, Storyweaver doit avoir :

- un **SPINE** chargé ;
- un fichier `Side_Quests_Campagne.md` (créé au besoin) ;
- des Références optionnelles (modulation).

Message en cas de manque :

- “Veuillez charger un SPINE pour permettre l’extraction S×L.”
- “Veuillez charger ou créer un fichier Side Quests pour archiver les résultats.”
---

### 6.7.2 La Matrice S×L — concept fondamental

La matrice S×L est la mécanique centrale du mode Side Quests.
Elle combine :

- **S (Sources)** : éléments concrets, déjà présents dans le SPINE ;
- **L (Liens)** : tensions, besoins ou prolongements narratifs issus de la campagne.

Une Side Quest = **1 Source S × 1 Lien L**

#### S — Sources (SPINE)

S1 · PNJ existants 
S2 · Lieux (villes, routes, ruines, campements) 
S3 · Événements locaux (fêtes, cérémonies, concours, élections) 
S4 · Interludes (voyages, pauses, missions tests) 
S5 · Objets, traces et vestiges déjà présents 
S6 · Anomalies ou singularités mentionnées dans le SPINE

> Une Source S ne provient jamais d’une invention. Elle doit être déjà décrite dans le SPINE.

#### L — Liens (prolongements narratifs)

L1 · Besoin local (social, politique, logistique) 
L2 · Fragment narratif (indice incomplet ou écho d’un mystère) 
L3 · Conséquence d’une scène canon 
L4 · Opportunité de progression (relation, réputation, ressource) 
L5 · Tonalité / rythme (ex : contemplatif, tendu, mystérieux) 
L6 · Tension externe (rivalité, menace, contrainte)

> Les Liens n’inventent rien : ils prolongent une dynamique déjà présente.

#### S×L — Exemples

- S1 × L1 → Un PNJ sollicite une aide ponctuelle. 
- S2 × L2 → Un lieu révèle un fragment incomplet. 
- S4 × L5 → Une halte contemplative dans le voyage. 
- S3 × L4 → Une fête ou cérémonie ouvre une progression sociale.
---

### 6.7.3 Workflow du Mode Side Quests

1) **Extraction SPINE** : PNJ, lieux, interludes, événements, arcs, tensions. 
2) **Lecture du SideQuest Ledger** (éviter doublons, suites possibles). 
3) **Références** (optionnelles) : modulation de ton/rythme. 
4) **Construction de la matrice S×L**. 
5) **Génération de 3 à 6 hooks** avec :
   - description courte,
   - lien narratif,
   - ramifications,
   - vecteurs concrets (objet/témoin/lieu),
   - emplacement idéal (interlude).
1) **Présentation au MJ** : validation / modification / rejet. 
2) **Archivage** dans `Side_Quests_Campagne.md`.

---

### 6.7.4 Garde-fous

- Zéro invention de lore. 
- Respect strict du SPINE. 
- Références = modulation, jamais création. 
- ≥ 1 Orientation MJ par hook (fail-forward). 
- Aucun indice critique unique. 
- Side Quests uniquement placées dans des interludes.
---
...
- `MODE: LORE ENGINE` ; `MODE: DRAMATURGIE` ; `MODE: DRAMATURGIE — PATHFINDER` ; `MODE: SCÈNES/AMBIANCES` ; `MODE: REFERENCE FILTER` ; `MODE: MECHANICS CONFIG` ; `MODE: EXPORT` ; `MODE: SIDE QUESTS`
- `MENU` ; `HELP`
